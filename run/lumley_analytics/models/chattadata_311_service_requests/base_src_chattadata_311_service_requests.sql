  ...
  create or replace view `lumley-analytics`.`chattadata_311_service_requests`.`base_src_chattadata_311_service_requests`
  OPTIONS(
      description="""This model processes and casts service request data fields from  src_chattadata_311_service_requests to appropriate data types from the original source data.\n"""
    )
  as -- The first CTE (source) selects all columns from the source table 'src_chattadata_311_service_requests'
